...
export const getPermissionsSeplag = (storageKey = "permissions_key"): string[] | null => {
  const json = localStorage.getItem(storageKey);
  return json ? JSON.parse(json) : null;
};

export const deepCloneMenuSeplag = (items: IMenuSeplag[]): IMenuSeplag[] =>
  items.map((item) => ({
    ...item,
    items: item.items ? deepCloneMenuSeplag(item.items) : undefined,
  }));
...
export const hasPermissionByRouteListSeplag = (
  ...
  const check = (item: IMenuSeplag) => {
    ...
    if (permissionKeys) {
      const has = hasPermissionByKeysSeplag(permissionKeys, permissions, adminRole);
