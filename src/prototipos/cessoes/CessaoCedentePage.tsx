...
export function PrototiposCessaoCedentePage({ modo = "cedente" }: { modo?: "cedente" | "seplag" }) {
  ...
  const persistir = (situacao: string, etapaAtual: string, acao: string, detalhe?: string) => {
    ...
    window.dispatchEvent(new Event("sigep-cessoes-updated"));
  ...
  return <PrototypeSystemPage nomeSistema="GESTÃO DE PESSOAS" ambienteSistema="Teste" menuItems={menuGestaoPessoas}>
    <div className="prototype-page-content prototype-page-content--white prototype-cedente-page">
      ...
      <CardSeplag title={isSeplag ? "Análise da cessão pela SEPLAG" : "Análise da cessão pelo órgão cedente"} cols="12" cardHeaderClassNames="prototype-regime-card prototype-ingressos-card"><div className="col-12 prototype-cedente-content">
        ...
        {isSeplag && <div className="prototype-seplag-checks"><strong>Verificações automáticas do protótipo</strong><span className={registro.matricula ? "is-ok" : "is-warning"}><i className={registro.matricula ? "pi pi-check-circle" : "pi pi-exclamation-circle"} /> Vínculo identificado</span><span className={registro.inicio && registro.fim ? "is-ok" : "is-warning"}><i className={registro.inicio && registro.fim ? "pi pi-check-circle" : "pi pi-exclamation-circle"} /> Período informado</span><span className={documentosConcluidos ? "is-ok" : "is-warning"}><i className={documentosConcluidos ? "pi pi-check-circle" : "pi pi-exclamation-circle"} /> Instrução obrigatória do Cedente</span></div>}

        <section className="prototype-cedente-section"><header><div><i className="pi pi-user" /><span><h4>{titulos.servidor}</h4><p>Identificação e vínculos abrangidos pela solicitação.</p></span></div><BadgeSeplag label={blocos.servidor.resultado === "APROVADO" && (!isSeplag || inconsistencias.servidor.length === 0) ? "Aprovado" : blocos.servidor.resultado === "CORRECAO" ? "Correção solicitada" : blocos.servidor.resultado === "REVISAR" ? "Revisar novamente" : "Pendente"} color={blocos.servidor.resultado === "APROVADO" && (!isSeplag || inconsistencias.servidor.length === 0) ? "#147441" : "#8a5c00"} bg={blocos.servidor.resultado === "APROVADO" && (!isSeplag || inconsistencias.servidor.length === 0) ? "#e2f5e8" : "#fff1cf"} border="transparent" size="sm" /></header><dl className="prototype-cedente-summary"><div><dt>Servidor</dt><dd>{registro.servidor}</dd></div><div><dt>Matrícula(s)</dt><dd>{registro.matricula}</dd></div><div><dt>Situação funcional</dt><dd>Consultar cadastro funcional</dd></div></dl>{renderDecisaoBloco("servidor")}</section>

        <section className="prototype-cedente-section"><header><div><i className="pi pi-building" /><span><h4>{titulos.destino}</h4><p>Órgão, unidade de exercício e cargo ou função.</p></span></div><BadgeSeplag label={blocos.destino.resultado === "APROVADO" && (!isSeplag || inconsistencias.destino.length === 0) ? "Aprovado" : blocos.destino.resultado === "CORRECAO" ? "Correção solicitada" : blocos.destino.resultado === "REVISAR" ? "Revisar novamente" : "Pendente"} color={blocos.destino.resultado === "APROVADO" && (!isSeplag || inconsistencias.destino.length === 0) ? "#147441" : "#8a5c00"} bg={blocos.destino.resultado === "APROVADO" && (!isSeplag || inconsistencias.destino.length === 0) ? "#e2f5e8" : "#fff1cf"} border="transparent" size="sm" /></header><dl className="prototype-cedente-summary"><div><dt>Cessionário</dt><dd>{registro.orgaoCessionario}</dd></div><div><dt>Unidade de exercício</dt><dd>{registro.unidadeDestino || "Não informada"}</dd></div><div><dt>Cargo ou função</dt><dd>{registro.cargoComissionado === "SIM" ? registro.cargoFuncao || "Sim — não especificado" : "Não"}</dd></div></dl>{renderDecisaoBloco("destino")}</section>

        <section className="prototype-cedente-section"><header><div><i className="pi pi-file-edit" /><span><h4>{titulos.dados}</h4><p>Hipótese, período, atividades, motivação e ônus.</p></span></div><BadgeSeplag label={blocos.dados.resultado === "APROVADO" && (!isSeplag || inconsistencias.dados.length === 0) ? "Aprovado" : blocos.dados.resultado === "CORRECAO" ? "Correção solicitada" : blocos.dados.resultado === "REVISAR" ? "Revisar novamente" : "Pendente"} color={blocos.dados.resultado === "APROVADO" && (!isSeplag || inconsistencias.dados.length === 0) ? "#147441" : "#8a5c00"} bg={blocos.dados.resultado === "APROVADO" && (!isSeplag || inconsistencias.dados.length === 0) ? "#e2f5e8" : "#fff1cf"} border="transparent" size="sm" /></header><dl className="prototype-cedente-summary"><div><dt>Hipótese</dt><dd>{registro.hipotese || "Não informada"}</dd></div><div><dt>Período</dt><dd>{registro.inicio} a {registro.fim}</dd></div><div><dt>Ônus</dt><dd>Órgão cessionário — sem reembolso</dd></div><div className="is-wide"><dt>Atividades</dt><dd>{registro.atividades || "Não informadas"}</dd></div><div className="is-wide"><dt>Motivação</dt><dd>{registro.motivacao || "Não informada"}</dd></div></dl>{renderDecisaoBloco("dados")}</section>

        <section className="prototype-cedente-section"><header><div><i className="pi pi-paperclip" /><span><h4>{titulos.documentos}</h4><p>Documentos vinculados pelo órgão cessionário.</p></span></div><BadgeSeplag label={blocos.documentos.resultado === "APROVADO" && (!isSeplag || inconsistencias.documentos.length === 0) ? "Aprovado" : blocos.documentos.resultado === "CORRECAO" ? "Correção solicitada" : blocos.documentos.resultado === "REVISAR" ? "Revisar novamente" : "Pendente"} color={blocos.documentos.resultado === "APROVADO" && (!isSeplag || inconsistencias.documentos.length === 0) ? "#147441" : "#8a5c00"} bg={blocos.documentos.resultado === "APROVADO" && (!isSeplag || inconsistencias.documentos.length === 0) ? "#e2f5e8" : "#fff1cf"} border="transparent" size="sm" /></header><dl className="prototype-cedente-summary"><div className="is-wide"><dt>Ofício/processo SIGADOC</dt><dd>{registro.processoSigadoc || "Não vinculado"}</dd></div></dl>{renderDecisaoBloco("documentos")}</section>

        <section className="prototype-cedente-section">
          <header><div><i className="pi pi-briefcase" /><span><h4>Instrução do órgão cedente</h4><p>{isSeplag ? "Consulte os documentos produzidos pelo Cedente durante a instrução." : "Vincule os documentos produzidos pelo Cedente antes do encaminhamento à SEPLAG."}</p></span></div></header>
          {isSeplag ? <div className="prototype-seplag-document-list">
            {([
              ["Solicitação de manifestação da unidade de lotação", docs.solicitacaoManifestacaoLotacao, false],
              ["Manifestação da unidade de lotação", docs.manifestacaoLotacao, true],
              ["Manifestação técnica da área setorial", docs.manifestacaoTecnica, true],
              ["Despacho do dirigente máximo", docs.despachoDecisao, true],
            ] as Array<[string, string, boolean]>).map(([titulo, referencia, obrigatorio]) => <article key={titulo} className={referencia ? "is-linked" : "is-missing"}><i className={referencia ? "pi pi-file-check" : "pi pi-exclamation-circle"} /><div><strong>{titulo}{obrigatorio ? " *" : " (opcional)"}</strong><span>{referencia ? `SIGADOC: ${referencia}` : "Documento não vinculado"}</span></div></article>)}
          </div> : <div className="grid prototype-cedente-documents">
            <TextFieldSeplag name="solicitacaoManifestacaoLotacao" label="Solicitação de manifestação da unidade de lotação (opcional)" value={docs.solicitacaoManifestacaoLotacao} onChange={(valor) => setDocs((atual) => ({ ...atual, solicitacaoManifestacaoLotacao: valor }))} placeholder="Número ou referência SIGADOC" disabled={somenteLeitura} cols="12 6" getFormErrorMessage={() => null} />
            <TextFieldSeplag name="manifestacaoLotacao" label="Manifestação da unidade de lotação" value={docs.manifestacaoLotacao} onChange={(valor) => setDocs((atual) => ({ ...atual, manifestacaoLotacao: valor }))} placeholder="Número ou referência SIGADOC" required disabled={somenteLeitura} cols="12 6" getFormErrorMessage={() => null} />
            <TextFieldSeplag name="manifestacaoTecnica" label="Manifestação técnica da área setorial" value={docs.manifestacaoTecnica} onChange={(valor) => setDocs((atual) => ({ ...atual, manifestacaoTecnica: valor }))} placeholder="Número ou referência SIGADOC" required disabled={somenteLeitura} cols="12 6" getFormErrorMessage={() => null} />
            <TextFieldSeplag name="despachoDecisao" label="Despacho do dirigente máximo" value={docs.despachoDecisao} onChange={(valor) => setDocs((atual) => ({ ...atual, despachoDecisao: valor }))} placeholder="Número ou referência SIGADOC" required disabled={somenteLeitura} cols="12 6" getFormErrorMessage={() => null} />
          </div>}
          {isSeplag ? <div className="prototype-seplag-central"><h4>Manifestação técnica do órgão central</h4><div className="grid prototype-cedente-documents">
            <TextFieldSeplag name="manifestacaoCentral" label="Número ou referência SIGADOC" value={manifestacaoCentral} onChange={setManifestacaoCentral} placeholder="Informe a referência" required disabled={somenteLeitura} cols="12 6" getFormErrorMessage={() => null} />
            <label className="col-12 md:col-6 prototype-seplag-conclusion"><span>Conclusão técnica *</span><select value={conclusaoCentral} onChange={(event) => setConclusaoCentral(event.target.value)} disabled={somenteLeitura}><option value="">Selecione...</option><option value="FAVORAVEL">Favorável ao encaminhamento</option><option value="COMPLEMENTACAO">Necessita complementação</option></select></label>
            <TextAreaFieldSeplag name="observacaoCentral" label="Observação ou fundamentação" value={observacaoCentral} onChange={setObservacaoCentral} rows={3} maxLength={2000} disabled={somenteLeitura} cols="12" getFormErrorMessage={() => null} />
          </div><div className="prototype-seplag-responsibility"><span><strong>Responsável:</strong> Analista SEPLAG (perfil simulado)</span><span><strong>Data:</strong> {new Date().toLocaleDateString("pt-BR")}</span></div></div> : <div className="prototype-cedente-note"><i className="pi pi-lock" /> A manifestação técnica do órgão central será produzida posteriormente pela SEPLAG e não é preenchida pelo Cedente.</div>}
        </section>

        {!somenteLeitura && <div className={podeAutorizar ? "prototype-cedente-readiness is-ready" : "prototype-cedente-readiness"}><i className={podeAutorizar ? "pi pi-check-circle" : "pi pi-exclamation-triangle"} /><span>{podeAutorizar ? (isSeplag ? "Análise completa. A solicitação pode ser encaminhada para publicação." : "Análise completa. A solicitação pode ser encaminhada à SEPLAG.") : `${pendencias} pendência(s). ${isSeplag ? "Marque os blocos como conformes e complete a manifestação técnica central." : "Aprove todos os blocos e complete os documentos do Cedente."}`}</span></div>}
