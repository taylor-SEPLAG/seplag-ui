...
type FiltrosQuadroComissionado = {
  busca: string;
  orgao: string;
  situacao: string;
};
...
const filtrosIniciais: FiltrosQuadroComissionado = {
  busca: "",
  orgao: "",
  situacao: "",
};
...
export function QuadroAutorizadoComissionadoLista() {
  ...
  const { control, reset, watch } = useForm<FiltrosQuadroComissionado>({
    defaultValues: filtrosIniciais,
  });
  const filtros = watch();
  const [detalhe, setDetalhe] = useState<QuadroLista | null>(null);
  const quadros = useMemo<QuadroLista[]>(
    () =>
      listarQuadrosComissionados().map((quadro, indice) => {
  ...
  const orgaos = useMemo(
    () => [...new Set(quadros.map((quadro) => quadro.orgao))].sort(),
    [quadros],
  );
  const filtrados = quadros.filter((quadro) => {
    ...
    return (
      (!termo ||
        `${quadro.codigo} ${quadro.nome}`
          .toLocaleLowerCase("pt-BR")
          .includes(termo)) &&
      (!filtros.orgao || quadro.orgao === filtros.orgao) &&
      (!filtros.situacao || quadro.situacao === filtros.situacao)
    );
  });
  const totais = quadros.reduce(
    (total, quadro) => ({
  ...
  const columns: ColumnMetaSeplag<QuadroLista>[] = [
    {
      ...
      sortable: true,
      body: (quadro) => (
        <div className="prototype-comissionados-quadro-main">
          <strong>{quadro.codigo}</strong>
          <small>{quadro.nome}</small>
        </div>
      ),
    },
    { header: "Órgão", field: "orgao", sortable: true },
    {
      ...
      field: "cargos",
      sortable: true,
      body: (quadro) => <strong>{quadro.cargos.toLocaleString("pt-BR")}</strong>,
    },
    {
      header: "Funções",
      field: "funcoes",
      sortable: true,
      body: (quadro) => <strong>{quadro.funcoes.toLocaleString("pt-BR")}</strong>,
    },
    {
      header: "Situação",
      field: "situacao",
      ...
      body: (quadro) => (
        <BadgeSeplag
          label={quadro.situacao}
          color={quadro.situacao === "Ativo" ? "#00843d" : "#8a5a00"}
          bg={quadro.situacao === "Ativo" ? "#dff3e8" : "#fff4d6"}
          size="xs"
        />
      ),
    },
  ...
  return (
    <main className="prototype-quadro-page prototype-quadro-page-current prototype-comissionados-quadro-page">
      <header className="prototype-quadro-header">
        <div>
          <h1>Quadro de Vagas Comissionados</h1>
          <p>Estruturas organizacionais e dotações autorizadas por órgão.</p>
        </div>
      </header>
      ...
      <section className="prototype-quadro-kpis">
        ...
        <Kpi label="Cargos em comissão autorizados" value={totais.cargos} icon="pi pi-briefcase" />
        <Kpi label="Funções de confiança autorizadas" value={totais.funcoes} icon="pi pi-users" />
        <Kpi label="Dotações registradas" value={totais.dotacoes} icon="pi pi-sitemap" />
      </section>
        ...
        <div className="prototype-quadro-filters prototype-quadro-library-filters prototype-comissionados-quadro-filters">
          <div className="prototype-quadro-spec-control">
            <TextFieldSeplag
              ...
              cols="12"
              icon="pi pi-search"
              placeholder="Nome ou código do quadro"
            />
          </div>
          <div className="prototype-quadro-spec-control">
            <DropdownFieldSeplag
              ...
              label="Órgão"
              cols="12"
              options={orgaos.map((value) => ({ label: value, value }))}
              optionLabel="label"
              optionValue="value"
              placeholder="Todos"
          ...
          <div className="prototype-quadro-spec-control">
            <BotaoLimparFiltroSeplag onClick={() => reset(filtrosIniciais)} />
          </div>
        </div>

        <div className="prototype-quadro-table-toolbar">
          <BotaoAdicionarSeplag label="Novo Quadro" onClick={novoQuadro} />
        </div>
        <div className="prototype-quadro-table prototype-quadro-library-table prototype-comissionados-quadro-table">
          <TablePaginadoSeplag<QuadroLista>
            dataKey="id"
            data={resultados(filtrados)}
            rows={10}
            ...
            actionHeader="Ações"
            renderBotoes={(quadro) => (
              <span className="prototype-quadro-actions prototype-comissionados-quadro-actions">
                <BotaoIconSeplag
                  icon="pi pi-eye"
                  aria-label={`Visualizar ${quadro.nome}`}
                  tooltip="Visualizar estrutura"
                  onClick={() => setDetalhe(quadro)}
                />
                <BotaoIconSeplag
                  icon="pi pi-pencil"
                  aria-label={`Editar ${quadro.nome}`}
                  tooltip="Editar quadro"
                  onClick={() => editarQuadro(quadro.id)}
                />

              </span>
            )}
            handleOnPageChange={() => undefined}
            emptyMessage="Nenhum quadro comissionado encontrado."
          />
        </div>
      </section>

      {detalhe && (
        <aside className="prototype-comissionados-quadro-detail" role="dialog" aria-label="Resumo do quadro">
          <header>
            <div>
              <span>QUADRO COMISSIONADO</span>
              <h2>{detalhe.nome}</h2>
            </div>
            <BotaoIconSeplag icon="pi pi-times" aria-label="Fechar resumo" tooltip="Fechar" onClick={() => setDetalhe(null)} />
          </header>
          <dl>
            <div><dt>Órgão</dt><dd>{detalhe.orgao}</dd></div>
            <div><dt>Níveis</dt><dd>{detalhe.niveis.length}</dd></div>
            <div><dt>Itens e subitens</dt><dd>{contarItens(detalhe.niveis)}</dd></div>
            <div><dt>Dotações</dt><dd>{detalhe.dotacoes}</dd></div>
            <div><dt>Cargos autorizados</dt><dd>{detalhe.cargos}</dd></div>
            <div><dt>Funções autorizadas</dt><dd>{detalhe.funcoes}</dd></div>
          </dl>
        </aside>
      )}
    </main>
...
function Kpi({ label, value, icon }: { label: string; value: number; icon: string }) {
  return <article><i className={icon} /><div><span>{label}</span><strong>{value.toLocaleString("pt-BR")}</strong></div></article>;
}

function contarItens(niveis: QuadroComissionadoSalvo["niveis"]) {
  const contar = (itens: ItemEstruturaComissionadaSalvo[]): number =>
    itens.reduce((total, item) => total + 1 + contar(item.subitens ?? []), 0);
  return niveis.reduce((total, nivel) => total + contar(nivel.itens ?? []), 0);
}