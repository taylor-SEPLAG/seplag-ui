...
export const gerarVagasDoQuadro=(quadro:QuadroAutorizadoRow):Vaga[]=>{
 ...
 return Array.from({length:quadro.autorizadas},(_,index)=>{
  const sequencial=index+1;const destinoLegal=quadro.quantitativosLegaisPorOrgao?.find((_,posicao,lista)=>sequencial<=lista.slice(0,posicao+1).reduce((total,item)=>total+item.quantidade,0));const orgaoDistribuicaoInicial=destinoLegal?.orgao??(quadro.formaDestinacaoLegal==="DEFINIDA_NA_LEI"&&quadro.orgaosDefinidosLei?.length===1?quadro.orgaosDefinidosLei[0]:undefined);const orgaoTitular=orgaoDistribuicaoInicial??(quadro.orgao!=="ESTADO DE MATO GROSSO"?quadro.orgao:"ESTADO DE MATO GROSSO");const destinacaoPrevistaLei=quadro.formaDestinacaoLegal==="DISTRIBUICAO_POSTERIOR"?"Pendente de ato de distribuição":orgaoDistribuicaoInicial?`Destinada pela lei ao órgão ${orgaoDistribuicaoInicial}`:"Órgãos permitidos definidos pela lei";const id=gerarIdentificadorVaga(orgaoTitular,quadro.cargo,sequencial);const estado:EstadoVaga=sequencial<=quadro.ocupadas?"OCUPADA":"DISPONIVEL";let situacaoLegal:SituacaoLegalVaga="REGULAR";
