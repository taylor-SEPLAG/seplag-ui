...
export const gerarNomeVaga=(orgao:string,cargo:string,sequencial:number)=>`${normalizar(orgao,6)}-${normalizar(cargo,10)}-${String(sequencial).padStart(5,"0")}`;
...
export const gerarVagasDoQuadro=(quadro:QuadroAutorizadoRow):Vaga[]=>{
 ...
 return Array.from({length:quadro.autorizadas},(_,index)=>{
  ...
  return{id,nome:orgaoDistribuicaoInicial?gerarNomeVaga(orgaoDistribuicaoInicial,quadro.cargo,sequencial):undefined,sequencial,quadroAutorizadoId:quadro.id,quadroCodigo:quadro.codigo,tipo,lei:quadro.ato,carreira:quadro.carreira,cargo:quadro.cargo,perfilProfissional:quadro.perfilProfissional,orgaoTitular,destinacaoPrevistaLei,orgaoDistribuicaoInicial,atoDistribuicaoInicial:orgaoDistribuicaoInicial?quadro.ato:undefined,inicioVigenciaDistribuicao:orgaoDistribuicaoInicial?quadro.inicioVigencia:undefined,criadaEm:"15/07/2026 08:00",inicioVigencia:quadro.inicioVigencia,estado,situacaoLegal,historico};
 });
};
