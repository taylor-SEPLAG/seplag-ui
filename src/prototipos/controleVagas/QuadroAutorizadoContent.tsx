...
import { carreirasBaseTemporaria, cargosBaseTemporaria, especialidadesBaseTemporaria, orgaosBaseTemporaria } from "./baseTemporaria";
...
const resumoOrgaos = (item: QuadroAutorizadoRow) => {
  if (item.formaDestinacaoLegal === "DISTRIBUICAO_POSTERIOR") return "Não distribuídas";
  const orgaos = orgaosDoQuadro(item).filter((orgao) => orgao !== "NÃO DISTRIBUÍDA");
  if (!orgaos.length) return "Não distribuídas";
...
function QuadroAutorizadoLista() {
  ...
  const filtrados = useMemo(() => {
    ...
    return quadros.filter((item) => (!termo || `${item.codigo} ${item.cargo} ${item.carreira} ${item.especialidade} ${orgaosDoQuadro(item).join(" ")}`.toLocaleLowerCase("pt-BR").includes(termo)) && (!orgao || orgaosDoQuadro(item).includes(orgao)) && (!tipo || item.tipoQuadro === tipo) && (!situacao || item.situacao === situacao));
...
function QuadroAutorizadoForm({ registro, novaVersao, onBack }: { registro?: QuadroAutorizadoRow; novaVersao: boolean; onBack: () => void }) {
  ...
  const [orgaosDefinidos, setOrgaosDefinidos] = useState<string[]>(registro?.orgaosDefinidosLei ? [...registro.orgaosDefinidosLei] : registro?.orgao && registro.orgao !== "NÃO DISTRIBUÍDA" ? [registro.orgao] : []);
  ...
  return <div className="prototype-quadro-page"><header className="prototype-quadro-header"><div><button className="prototype-quadro-back" onClick={onBack}><i className="pi pi-arrow-left" /> Quadro Autorizado</button><h1>{titulo}</h1><p>{novaVersao ? `${registro?.codigo} • versão atual ${registro?.versao}` : "Preencha os dados que fundamentam o quantitativo autorizado."}</p></div><span className="prototype-quadro-form-state">{novaVersao ? "Nova versão" : registro ? "Edição" : "Nova autorização"}</span></header>
    ...
    <form onSubmit={submit} className="prototype-quadro-form"><section><header><i className="pi pi-briefcase" /><div><h2>Identificação do quadro</h2><p>Combinação utilizada para controlar o quantitativo.</p></div></header><div className="prototype-quadro-fields"><Field label="Tipo de quadro *"><DropdownPesquisa value={form.tipoQuadro} onChange={(valor) => set("tipoQuadro", valor)} options={["Efetivo", "Comissionado"]} /></Field><Field label="Tipo de vínculo *"><DropdownPesquisa value={form.vinculo} onChange={(valor) => set("vinculo", valor)} options={["Servidor efetivo", "Exclusivamente comissionado"]} /></Field><Field label="Regime jurídico"><DropdownPesquisa value={form.regime} onChange={(valor) => set("regime", valor)} options={["Estatutário", "Administrativo", "Celetista"]} /></Field><Field label="Carreira"><DropdownPesquisa value={form.carreira} onChange={(valor) => set("carreira", valor)} options={carreirasBaseTemporaria.map((item) => ({ label: item.situacaoLegal === "EM_EXTINCAO" ? item.nome + " — Em extinção" : item.nome, value: item.nome }))} /></Field><Field label="Cargo *" wide><DropdownPesquisa value={form.cargo} onChange={(valor) => set("cargo", valor)} options={cargosBaseTemporaria.map((item) => ({ label: item.situacaoLegal === "EM_EXTINCAO" ? item.nome + " — Em extinção" : item.nome, value: item.nome }))} /></Field><Field label="Especialidade"><DropdownPesquisa value={form.especialidade} onChange={(valor) => set("especialidade", valor)} options={especialidadesBaseTemporaria.map((item) => item.nome)} placeholder="Sem especialidades cadastradas" disabled={!especialidadesBaseTemporaria.length} /></Field></div></section>      <section><header><i className="pi pi-building" /><div><h2>{novaVersao ? "Nova configuração legal" : "Abrangência e quantitativo"}</h2><p>{novaVersao ? "Informe somente o quantitativo e a alocação que passarão a valer." : "Defina o total autorizado e como a legislação alocou as vagas."}</p></div></header>
        <div className="prototype-quadro-scope">
          ...
          {modoAbrangencia === "SEM_ORGAOS" && <div className="prototype-quadro-destination-info"><i className="pi pi-info-circle" /><span>As vagas serão criadas como <strong>não distribuídas</strong>. A destinação será registrada posteriormente no menu Distribuição.</span></div>}
