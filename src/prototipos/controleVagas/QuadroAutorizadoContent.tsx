...
import { quadroActionSpecifications, quadroBusinessItems, quadroColumnSpecifications, quadroFilterSpecifications, quadroKpiSpecifications, quadroScreenSpecification } from "./QuadroAutorizadoSpecifications";
...
function QuadroAutorizadoLista() {
  ...
  return <SpecificationMode screen={quadroScreenSpecification} businessItems={quadroBusinessItems}><div className="prototype-quadro-page">
    <header className="prototype-quadro-header"><SpecArea metadata={quadroScreenSpecification}><div><h1>Quadro Autorizado</h1><p>Quantitativos autorizados por cargo, vínculo e órgão.</p></div></SpecArea><SpecArea metadata={quadroActionSpecifications["Nova autorização"]}><button className="prototype-quadro-primary" onClick={() => navigate(`${BASE_PATH}/novo`)}><i className="pi pi-plus" /> Nova autorização</button></SpecArea></header>
    
    <section className="prototype-quadro-kpis"><SpecArea metadata={quadroKpiSpecifications.Autorizadas}><article><i className="pi pi-file-check" /><div><span>Autorizadas</span><strong>{totais.autorizadas.toLocaleString("pt-BR")}</strong></div></article></SpecArea><SpecArea metadata={quadroKpiSpecifications.Ocupadas}><article><i className="pi pi-users" /><div><span>Ocupadas</span><strong>{totais.ocupadas.toLocaleString("pt-BR")}</strong></div></article></SpecArea><SpecArea metadata={quadroKpiSpecifications.Comprometidas}><article><i className="pi pi-clock" /><div><span>Comprometidas</span><strong>{totais.comprometidas.toLocaleString("pt-BR")}</strong></div></article></SpecArea><SpecArea metadata={quadroKpiSpecifications.Disponíveis}><article className="is-available"><i className="pi pi-check-circle" /><div><span>Disponíveis</span><strong>{totais.disponiveis.toLocaleString("pt-BR")}</strong></div></article></SpecArea></section>
    <section className="prototype-quadro-card">
      <div className="prototype-quadro-filters"><SpecArea metadata={quadroFilterSpecifications["Código, cargo ou carreira"]}><label className="is-wide"><span>Código, cargo ou carreira</span><div><i className="pi pi-search" /><input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Pesquisar no quadro" /></div></label></SpecArea><SpecArea metadata={quadroFilterSpecifications.Órgão}><label><span>Órgão</span><select value={orgao} onChange={(e) => setOrgao(e.target.value)}><option value="">Todos</option>{[...new Set(quadros.flatMap(orgaosDoQuadro))].map((i) => <option key={i}>{i}</option>)}</select></label></SpecArea><SpecArea metadata={quadroFilterSpecifications["Tipo de quadro"]}><label><span>Tipo de quadro</span><select value={tipo} onChange={(e) => setTipo(e.target.value)}><option value="">Todos</option>{[...new Set(quadros.map((i) => i.tipoQuadro))].map((i) => <option key={i}>{i}</option>)}</select></label></SpecArea><SpecArea metadata={quadroFilterSpecifications.Situação}><label><span>Situação</span><select value={situacao} onChange={(e) => setSituacao(e.target.value)}><option value="">Todas</option>{[...new Set(quadros.map((i) => i.situacao))].map((i) => <option key={i}>{i}</option>)}</select></label></SpecArea><SpecArea metadata={quadroFilterSpecifications["Data de referência"]}><label><span>Data de referência</span><input type="date" value={dataReferencia} onChange={(e) => setDataReferencia(e.target.value)} /></label></SpecArea><SpecArea metadata={quadroFilterSpecifications.Limpar}><button onClick={limpar}><i className="pi pi-filter-slash" /> Limpar</button></SpecArea></div>
      <div className="prototype-quadro-table"><table><thead><tr>{([ ["quadro", "Quadro", false], ["cargo", "Cargo/Função", false], ["orgao", "Órgão", false], ["autorizadas", "Autorizadas", true], ["ocupadas", "Ocupadas", true], ["disponiveis", "Disponíveis", true], ["vigencia", "Vigência", false], ["situacao", "Situação", false] ] as const).map(([campo, rotulo, numerico]) => <th key={campo} className={numerico ? "is-number" : undefined}><SpecArea metadata={quadroColumnSpecifications[rotulo]}><button type="button" className="prototype-quadro-sort" onClick={() => alternarOrdenacao(campo)} aria-label={`Ordenar por ${rotulo}`}><span>{rotulo}</span><i className={`pi ${ordenacao?.campo === campo ? ordenacao.direcao === "asc" ? "pi-sort-up-fill" : "pi-sort-down-fill" : "pi-sort-alt"}`} /></button></SpecArea></th>)}<th><SpecArea metadata={quadroColumnSpecifications.Ações}><span>Ações</span></SpecArea></th></tr></thead><tbody>{filtrados.map((item) => <tr key={item.id}><td><button className="prototype-quadro-link" onClick={() => setVisualizado(item)}>{item.codigo}</button><small>Versão {item.versao}</small></td><td><strong>{item.cargo}</strong><small>{item.perfilProfissional || item.vinculo}</small></td><td>{resumoOrgaos(item)}{orgaosDoQuadro(item).length > 1 && <small>{orgaosDoQuadro(item).join(" • ")}</small>}</td><td className="is-number">{item.autorizadas}</td><td className="is-number">{item.ocupadas}</td><td className={`is-number ${saldo(item) <= 0 ? "is-danger" : "is-positive"}`}><strong>{Math.max(0, saldo(item))}</strong></td><td>{item.inicioVigencia || "Não informada"}<small>{item.fimVigencia ? `até ${item.fimVigencia}` : "sem término"}</small></td><td><span className={`prototype-quadro-status ${situacaoClass[item.situacao]}`}>{item.situacao}</span></td><td><div className="prototype-quadro-actions"><button title="Visualizar" onClick={() => setVisualizado(item)}><i className="pi pi-eye" /></button><button title={item.situacao === "Vigência futura" ? "Editar antes da vigência" : "Edição indisponível após a vigência"} disabled={item.situacao !== "Vigência futura"} onClick={() => navigate(`${BASE_PATH}/${item.id}/editar`)}><i className="pi pi-pencil" /></button>{item.situacao === "Vigência futura" && <button title="Excluir antes da vigência" onClick={() => excluirQuadroFuturo(item)}><i className="pi pi-trash" /></button>}{item.situacao === "Vigente" && <button title="Distribuir vagas deste quadro" onClick={() => navigate(`${CONTROLE_VAGAS_BASE_PATH}/distribuicao?quadro=${item.id}`)}><i className="pi pi-sitemap" /></button>}{item.situacao === "Vigente" && <button title="Criar nova versão" onClick={() => navigate(`${BASE_PATH}/${item.id}/nova-versao`)}><i className="pi pi-plus" /></button>}</div></td></tr>)}</tbody></table></div>
