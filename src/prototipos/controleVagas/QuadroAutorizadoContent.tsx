...
function QuadroAutorizadoForm({ registro, novaVersao, onBack }: { registro?: QuadroAutorizadoRow; novaVersao: boolean; onBack: () => void }) {
  ...
  const [carreirasSugeridas, setCarreirasSugeridas] = useState<string[]>([]);
  const [cargosSugeridos, setCargosSugeridos] = useState<string[]>([]);
  ...
  const filtrarCatalogo = (consulta: string, itens: typeof carreirasBaseTemporaria) => {
    const termo = consulta.trim().toLocaleLowerCase("pt-BR");
    return itens.filter((item) => !termo || item.nome.toLocaleLowerCase("pt-BR").includes(termo)).map((item) => item.nome);
  };
  const submit = (event: FormEvent) => {
    ...
    const atual=controleVagasStore.getState();const novoId=novaVersao||!registro?Math.max(0,...atual.quadros.map((item)=>item.id))+1:registro.id;const codigo=novaVersao?registro!.codigo:registro?.codigo??`QA-${String(novoId).padStart(4,"0")}`;const situacao=form.inicioVigencia>"2026-07-17"?"Vigência futura" as const:"Vigente" as const;const dataBr=(data:string)=>data?data.split("-").reverse().join("/"):"";
    const quadro:QuadroAutorizadoRow={id:novoId,codigo,tipoQuadro:form.tipoQuadro as QuadroAutorizadoRow["tipoQuadro"],vinculo:form.vinculo,regime:form.regime,carreira:form.carreira,cargo:form.cargo,especialidade:form.especialidade,orgao:form.orgao,abrangencia:form.abrangencia,autorizadas:Number(form.quantidade),ocupadas:0,comprometidas:0,bloqueadas:0,inicioVigencia:dataBr(form.inicioVigencia),fimVigencia:dataBr(form.fimVigencia),ato:form.numeroAto,processo:form.processo,situacao,versao:novaVersao?(registro?.versao??0)+1:registro?.versao??1,atualizadoEm:"17/07/2026"};
    controleVagasStore.set("quadros",(itens)=>novaVersao||!registro?[...itens,quadro]:itens.map((item)=>item.id===registro.id?quadro:item));
    if(!registro&&!novaVersao&&situacao==="Vigente")controleVagasStore.set("vagas",(itens)=>[...itens,...gerarVagasDoQuadro(quadro)]);
  ...
  return <div className="prototype-quadro-page"><header className="prototype-quadro-header"><div><button className="prototype-quadro-back" onClick={onBack}><i className="pi pi-arrow-left" /> Quadro Autorizado</button><h1>{titulo}</h1><p>{novaVersao ? `${registro?.codigo} • versão atual ${registro?.versao}` : "Preencha os dados que fundamentam o quantitativo autorizado."}</p></div><span className="prototype-quadro-form-state">{novaVersao ? "Nova versão" : registro ? "Edição" : "Nova autorização"}</span></header>
    ...
    <form onSubmit={submit} className="prototype-quadro-form"><section><header><i className="pi pi-briefcase" /><div><h2>Identificação do quadro</h2><p>Combinação utilizada para controlar o quantitativo.</p></div></header><div className="prototype-quadro-fields"><Field label="Tipo de quadro *"><select value={form.tipoQuadro} onChange={(e) => set("tipoQuadro", e.target.value)}><option value="">Selecione</option><option>Efetivo</option><option>Comissionado</option></select></Field><Field label="Tipo de vínculo *"><select value={form.vinculo} onChange={(e) => set("vinculo", e.target.value)}><option value="">Selecione</option><option>Servidor efetivo</option><option>Exclusivamente comissionado</option></select></Field><Field label="Regime jurídico"><select value={form.regime} onChange={(e) => set("regime", e.target.value)}><option value="">Selecione</option><option>Estatutário</option><option>Administrativo</option><option>Celetista</option></select></Field><Field label="Carreira"><SeplagAutoComplete inputId="quadro-carreira" className="w-full" value={form.carreira} suggestions={carreirasSugeridas} placeholder="Digite para buscar" dropdown forceSelection completeMethod={(consulta) => setCarreirasSugeridas(filtrarCatalogo(consulta, carreirasBaseTemporaria))} onChange={(e) => set("carreira", String(e.value ?? ""))} /></Field><Field label="Cargo *" wide><SeplagAutoComplete inputId="quadro-cargo" className="w-full" value={form.cargo} suggestions={cargosSugeridos} placeholder="Digite para buscar" dropdown forceSelection maxRenderedItems={100} completeMethod={(consulta) => setCargosSugeridos(filtrarCatalogo(consulta, cargosBaseTemporaria))} onChange={(e) => set("cargo", String(e.value ?? ""))} /></Field><Field label="Especialidade"><select value={form.especialidade} onChange={(e) => set("especialidade", e.target.value)}><option value="">Sem especialidade informada</option>{especialidadesBaseTemporaria.map((item) => <option key={item.id} value={item.nome}>{item.nome}</option>)}</select></Field></div></section>
      ...
      <footer className="prototype-quadro-form-actions"><button type="button" className="is-cancel" onClick={onBack}>Cancelar</button><button type="submit" className="is-submit"><i className="pi pi-save" /> Salvar</button></footer>
    </form></div>;
}
