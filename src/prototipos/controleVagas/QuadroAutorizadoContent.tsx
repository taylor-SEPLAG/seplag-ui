...
function QuadroAutorizadoLista() {
  ...
  const filtrados = useMemo(() => {
    ...
    return quadros.filter((item) => (!termo || `${item.codigo} ${item.cargo} ${item.carreira} ${item.perfilProfissional} ${orgaosDoQuadro(item).join(" ")}`.toLocaleLowerCase("pt-BR").includes(termo)) && (!orgao || orgaosDoQuadro(item).includes(orgao)) && (!tipo || item.tipoQuadro === tipo) && (!situacao || item.situacao === situacao));
