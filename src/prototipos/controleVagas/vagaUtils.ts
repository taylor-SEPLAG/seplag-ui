...
export const gerarVagasDoQuadro=(quadro:QuadroAutorizadoRow):Vaga[]=>{
 ...
 return Array.from({length:quadro.autorizadas},(_,index)=>{
  const sequencial=index+1;const id=gerarIdentificadorVaga(quadro.orgao,quadro.cargo,sequencial);const estado:EstadoVaga=sequencial<=quadro.ocupadas?"OCUPADA":"DISPONIVEL";let situacaoLegal:SituacaoLegalVaga="REGULAR";
  if(quadro.codigo==="QA-0002"&&sequencial===quadro.autorizadas)situacaoLegal="DECISAO_JUDICIAL";
  if(quadro.codigo==="QA-0001"&&sequencial>quadro.autorizadas-3)situacaoLegal="EM_EXTINCAO";
