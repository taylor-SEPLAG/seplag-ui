...
export const gerarVagasDoQuadro=(quadro:QuadroAutorizadoRow):Vaga[]=>{
 ...
 return Array.from({length:quadro.autorizadas},(_,index)=>{
  const sequencial=index+1;const destinoLegal=quadro.quantitativosLegaisPorOrgao?.find((_,posicao,lista)=>sequencial<=lista.slice(0,posicao+1).reduce((total,item)=>total+item.quantidade,0));const orgaoVaga=destinoLegal?.orgao??(quadro.formaDestinacaoLegal==="DISTRIBUICAO_POSTERIOR"?"NÃO DISTRIBUÍDA":quadro.formaDestinacaoLegal==="DEFINIDA_NA_LEI"&&quadro.orgaosDefinidosLei?.length===1?quadro.orgaosDefinidosLei[0]:quadro.formaDestinacaoLegal? "NÃO DISTRIBUÍDA":quadro.orgao);const id=gerarIdentificadorVaga(orgaoVaga,quadro.cargo,sequencial);const estado:EstadoVaga=sequencial<=quadro.ocupadas?"OCUPADA":"DISPONIVEL";let situacaoLegal:SituacaoLegalVaga="REGULAR";
  ...
  return{id,sequencial,quadroAutorizadoId:quadro.id,quadroCodigo:quadro.codigo,tipo,lei:quadro.ato,carreira:quadro.carreira,cargo:quadro.cargo,orgaoTitular:orgaoVaga,criadaEm:"15/07/2026 08:00",inicioVigencia:quadro.inicioVigencia,estado,situacaoLegal,historico};
 });
};
