...
export const vagasKpiSpecifications: Record<string, SpecificationMetadata> = {
  "Vagas distribuídas": spec("CV-VAG-KPI-001", "Vagas distribuídas", "Quantidade de vagas com distribuição formal no quadro consultado.", "Somente vagas distribuídas recebem nome da vaga e compõem a listagem.", "Vaga[] e MovimentoVagaIndividual[].", "integer", "KPI"),
  "Pendentes de distribuição": spec("CV-VAG-KPI-006", "Pendentes de distribuição", "Quantidade de vagas autorizadas que ainda não possuem distribuição formal.", "Exibir somente o total no card; essas vagas não recebem nome da vaga e não aparecem na tabela.", "Vaga[] e posição de distribuição.", "integer", "KPI"),
...
export const vagasFilterSpecifications: Record<string, SpecificationMetadata> = {
  Identificador: spec("CV-VAG-FLT-001", "Nome da vaga", "Localizar uma vaga pelo seu nome individual.", "Pesquisar exclusivamente o nome da vaga; o olho controla a coluna sem alterar o filtro.", "Vaga.id.", "string", "campo de busca"),
  ...
  Ocupação: spec("CV-VAG-FLT-008", "Ocupação", "Consultar vaga por condição e histórico de ocupação.", "Distinguir ocupante atual, ocupante anterior e ausência de histórico.", "OcupacaoVaga[].", "enum | vazio", "dropdown"),
