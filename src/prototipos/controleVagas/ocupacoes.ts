...
export function registrarEfetivoExercicio(vaga:Vaga,dados:DadosNovaOcupacao):{vaga:Vaga;ocupacao:OcupacaoVaga}{
 if(vaga.situacaoLegal==="EM_EXTINCAO"||vaga.situacaoLegal==="EXTINTA"){
  throw new Error("A vaga está em extinção e não permite uma nova ocupação.");
 }
...
export function encerrarOcupacao(vaga:Vaga,ocupacao:OcupacaoVaga,data:string,motivo:string,processo?:string):{vaga:Vaga;ocupacao:OcupacaoVaga}{
 ...
 const extinguir=vaga.situacaoLegal==="EM_EXTINCAO";
 const historico=[...vaga.historico,{id:`HIS-${vaga.id}-${String(vaga.historico.length+1).padStart(3,"0")}`,vagaId:vaga.id,ocorridoEm:"16/07/2026 15:00",dataEfeito:data,tipo:"ALTERACAO_ESTADO" as const,titulo:"Ocupação nominal encerrada",descricao:`Vínculo ${ocupacao.vinculoId} liberou a vaga: ${motivo}.`,estadoAnterior:"OCUPADA" as const,estadoPosterior:"DISPONIVEL" as const,origem:"Vida Funcional",usuario:"Integração SIGEP",processo},...(extinguir?[{id:`HIS-${vaga.id}-${String(vaga.historico.length+2).padStart(3,"0")}`,vagaId:vaga.id,ocorridoEm:"16/07/2026 15:00",dataEfeito:data,tipo:"ALTERACAO_LEGAL" as const,titulo:"Vaga extinta após vacância",descricao:"A extinção progressiva foi efetivada com a liberação da vaga.",situacaoLegalAnterior:"EM_EXTINCAO" as const,situacaoLegalPosterior:"EXTINTA" as const,origem:"Quadro Legal",usuario:"Integração SIGEP",processo}]:[])];
 return{vaga:{...vaga,estado:"DISPONIVEL",situacaoLegal:extinguir?"EXTINTA":vaga.situacaoLegal,historico},ocupacao:atualizada};
}
