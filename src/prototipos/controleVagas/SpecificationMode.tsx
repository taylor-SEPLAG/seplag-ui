...
type BusinessStatus = "PENDENTE" | "APROVADO" | "RESSALVA" | "AJUSTE" | "DUVIDA";
...
const SpecificationContext = createContext<SpecificationContextValue>({ active: false, mode: null, statuses: {}, select: () => undefined });
const statusLabel:Record<BusinessStatus,string>={PENDENTE:"Pendente de validação",APROVADO:"Aprovado",RESSALVA:"Aprovado com ressalva",AJUSTE:"Ajuste solicitado",DUVIDA:"Dúvida registrada"};

export function SpecificationMode({ children, screen, businessItems = [] }: { children: ReactNode; screen: SpecificationMetadata; businessItems?: SpecificationMetadata[] }) {
  ...
  const toggleMode = (next: ViewMode) => { setMode((current) => current === next ? null : next); setSelected(null); setPanelCollapsed(false); setDraft(""); };
  ...
  return <SpecificationContext.Provider value={{ active, mode, statuses, select }}>
    <div className={`${active ? "prototype-spec-root is-active" : "prototype-spec-root"}${panelCollapsed ? " is-panel-collapsed" : ""}`}>
      {children}
      <div className="prototype-view-toggles" role="group" aria-label="Modos de visualização do protótipo">
...
export function SpecArea({ metadata, children }: { metadata: SpecificationMetadata; children: ReactElement }) {
  ...
  return cloneElement(children, {
    className: `${original.className ?? ""} prototype-spec-target ${mode === "BUSINESS" ? `is-business status-${status.toLowerCase()}` : "is-developer"}`.trim(),
    "data-spec-code": mode === "BUSINESS" ? metadata.title : metadata.id,
    title: `${mode === "BUSINESS" ? "Validar" : "Ver especificação"}: ${metadata.title}`,
    onClickCapture: (event: MouseEvent) => { event.preventDefault(); event.stopPropagation(); select(metadata); },
  } as never);
}
...
function PrototypePanel(props: PanelProps) {
  if (props.collapsed) return <aside className="prototype-spec-panel is-collapsed" aria-label="Painel de visualização recolhido"><button className="prototype-spec-expand" type="button" onClick={props.onToggle} title="Expandir painel"><i className="pi pi-angle-left"/><span>{props.mode === "BUSINESS" ? "Validação" : "Especificação"}</span></button></aside>;
  return props.mode === "BUSINESS" ? <BusinessPanel {...props}/> : <DeveloperPanel metadata={props.metadata} onToggle={props.onToggle}/>;
}

function DeveloperPanel({ metadata, onToggle }: { metadata: SpecificationMetadata; onToggle: () => void }) {
  return <aside className="prototype-spec-panel" aria-label="Especificação do componente">
    <PanelHeader eyebrow={metadata.id} title={metadata.title} onToggle={onToggle}/>
...
function BusinessPanel(props: PanelProps) {
  ...
  return <aside className="prototype-spec-panel prototype-business-panel" aria-label="Validação da área de negócio">
    ...
    {props.screenSelected ? <>
      ...
      <div className={`prototype-business-status status-${props.status.toLowerCase()}`}><span>Situação</span><strong>{statusLabel[props.status]}</strong></div>
      <section><h3>O que este item representa</h3><p className="prototype-business-intro">{props.metadata.description}</p></section>
      <section><h3>O que validar</h3><ul className="prototype-business-checklist"><li>O nome e o significado estão adequados?</li><li>Esta informação é necessária para a decisão?</li><li>Os filtros e o comportamento esperado estão corretos?</li><li>A ação executada ao clicar faz sentido?</li></ul></section>
      <section><h3>Sua avaliação</h3><div className="prototype-business-actions"><button className="approve" onClick={()=>props.onStatus("APROVADO")}><i className="pi pi-check"/>Aprovar</button><button className="adjust" onClick={()=>props.onStatus("AJUSTE")}><i className="pi pi-pencil"/>Solicitar ajuste</button><button className="question" onClick={()=>props.onStatus("DUVIDA")}><i className="pi pi-question-circle"/>Dúvida</button></div><textarea value={props.draft} onChange={(event)=>props.onDraft(event.target.value)} placeholder="Escreva um comentário..." rows={3}/><button className="prototype-business-comment" onClick={props.onComment} disabled={!props.draft.trim()}><i className="pi pi-comment"/>Adicionar comentário</button>{props.comments.length>0&&<div className="prototype-business-comments">{props.comments.map((comment,index)=><article key={`${comment}-${index}`}><strong>Comentário da validação</strong><p>{comment}</p><small>Registrado nesta sessão</small></article>)}</div>}</section>
    </>}
  </aside>;
}

function PanelHeader({ eyebrow, title, onToggle }:{eyebrow:string;title:string;onToggle:()=>void}){return <header><div><span>{eyebrow}</span><h2>{title}</h2></div><button type="button" onClick={onToggle} title="Recolher painel"><i className="pi pi-angle-right"/></button></header>}
