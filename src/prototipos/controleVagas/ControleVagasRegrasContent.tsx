...
interface RegraEvento {
  ...
  evento: string;
  origem: string;
  impacto: ImpactoRegra;
  comportamento: string;
  validacao: "Provisória" | "Confirmada";
}

const regrasIniciais: RegraEvento[] = [
  ...
  { id: 3, evento: "Comprometimento da vaga", origem: "Sistema externo", impacto: "Comprometimento", comportamento: "Reduz o saldo disponível sem ocupar a vaga", validacao: "Provisória" },
  { id: 4, evento: "Cancelamento do comprometimento", origem: "Sistema externo", impacto: "Liberação", comportamento: "Devolve a quantidade comprometida ao saldo", validacao: "Provisória" },
  { id: 5, evento: "Ocupação confirmada", origem: "Vida funcional", impacto: "Ocupação", comportamento: "Converte comprometimento em ocupação", validacao: "Provisória" },
  ...
  { id: 7, evento: "Vacância ou desligamento", origem: "Vida funcional", impacto: "Liberação", comportamento: "Reduz a ocupação e recompõe o saldo", validacao: "Provisória" },
  ...
  { id: 9, evento: "Desbloqueio", origem: "Controle de Vagas", impacto: "Bloqueio", comportamento: "Recompõe o saldo anteriormente bloqueado", validacao: "Provisória" },
  { id: 10, evento: "Transferência", origem: "Controle de Vagas", impacto: "Liberação", comportamento: "Move quantitativo entre distribuições sem alterar o total", validacao: "Provisória" },
...
const pendencias = [
  "Marco que transforma uma vaga em ocupada",
  "Existência de comprometimento formal antes da ocupação",
  "Evento e data que liberam a vaga",
  "Dimensões obrigatórias do saldo por modalidade",
  "Modalidades que exigem vaga individualmente numerada",
  "Tratamento definitivo de extraquadro e decisão judicial",
];
  ...
  return (
    <div className="prototype-vagas-rules-page">
      ...
      <section className="prototype-vagas-summary" aria-label="Resumo da configuração">
        ...
        <article><span className="is-orange"><i className="pi pi-question-circle" /></span><div><strong>{pendencias.length}</strong><small>Pontos para validação</small></div></article>
      </section>
      ...
      <nav className="prototype-vagas-tabs" aria-label="Seções da configuração">
        ...
        <button className={aba === "pendencias" ? "is-active" : ""} onClick={() => setAba("pendencias")}>Pendências <span>{pendencias.length}</span></button>
      </nav>
      ...
      {aba === "pendencias" && <section className="prototype-vagas-panel"><div className="prototype-vagas-panel-heading"><div><h2>Pontos para validação com a área de negócio</h2><p>O protótipo utiliza premissas provisórias enquanto estas definições estiverem pendentes.</p></div></div><ol className="prototype-vagas-pending-list">{pendencias.map((item, index) => <li key={item}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{item}</strong><small>Aguardando validação</small></div><i className="pi pi-chevron-right" /></li>)}</ol></section>}
