...
function QuadrosContratosTemporariosLista() {
  ...
  const vagasPrevistas = quadros.reduce(
    (total, quadro) => total + quadro.cargos.reduce((subtotal, cargo) => subtotal + cargo.quantidadeVagas, 0),
    0,
  );
  ...
  return (
    <div className="prototype-temporarios-quadro-page prototype-temporarios-quadro-page-current">
      ...
      <section className="prototype-temporarios-quadro-kpis">
        <article><i className="pi pi-sitemap" /><div><span>Processos seletivos cadastrados</span><strong>{processosSeletivos}</strong></div></article>
        <article><i className="pi pi-file-check" /><div><span>Quadros temporários criados</span><strong>{quadros.length}</strong></div></article>
        <article><i className="pi pi-briefcase" /><div><span>Cargos com vagas temporárias</span><strong>{cargosControlados}</strong></div></article>
        <article><i className="pi pi-clock" /><div><span>Vagas temporárias previstas</span><strong>{vagasPrevistas}</strong></div></article>
        <article><i className="pi pi-users" /><div><span>Vagas temporárias reais</span><strong>{vagasReais}</strong></div></article>
      </section>
      ...
      <section className="prototype-temporarios-quadro-card prototype-temporarios-quadro-table-card">
        ...
        <div className="prototype-temporarios-quadro-table prototype-temporarios-quadro-list-table">
          <table><thead><tr><th>Quadro <i className="pi pi-sort-alt" /></th><th>Processo seletivo <i className="pi pi-sort-alt" /></th><th>Órgão responsável <i className="pi pi-sort-alt" /></th><th>Vigência <i className="pi pi-sort-alt" /></th><th>Cargos <i className="pi pi-sort-alt" /></th><th>Vagas previstas <i className="pi pi-sort-alt" /></th><th>Situação <i className="pi pi-sort-alt" /></th><th>Ações</th></tr></thead><tbody>{filtrados.map((q) => <tr key={q.id}><td><button className="prototype-temporarios-quadro-link" onClick={() => setSelecionado(q.id)}>{q.codigo}</button><small>{q.certame.numeroEditalOrgao}</small></td><td><strong>{q.certame.nomeEdital}</strong><small>{q.certame.situacaoAtual}</small></td><td>{q.certame.setor}</td><td>{q.dataAtivacao ?? "Não informada"}</td><td><button type="button" className="prototype-temporarios-quadro-cargos-action" aria-label={`Ver cargos do quadro ${q.codigo}`} onClick={() => setCargosSelecionado(q.id)}>{q.cargos.length}</button></td><td>{q.cargos.reduce((total, cargo) => total + cargo.quantidadeVagas, 0)}{q.cargos.some((cargo) => cargo.aceitaCadastroReserva) ? " + CR" : ""}</td><td><SituacaoTemporarioBadge situacao={q.situacao} /></td><td><div className="prototype-temporarios-quadro-actions"><BotaoIconSeplag className="prototype-temporarios-quadro-view-action" icon="pi pi-eye" tooltip="Visualizar quadro" aria-label={`Visualizar quadro ${q.codigo}`} style={{ width: 32, height: 32, padding: 0, border: 0, borderRadius: 4, backgroundColor: "#0784d3" }} onClick={() => setSelecionado(q.id)} /></div></td></tr>)}</tbody></table>
...
function CargosQuadroTemporarioModal({ quadro, onClose }: { quadro: QuadroTemporarioCadastro; onClose: () => void }) {
...
function SituacaoTemporarioBadge({ situacao }: { situacao: "ATIVO" | "EXTINTO" | "ENCERRADO" }) {
  const meta = situacao === "EXTINTO"
    ...
    : situacao === "ENCERRADO"
      ? { label: "Encerrado", color: "#64748b", bg: "#f1f5f9" }
      : { label: "Ativo", color: "#00843d", bg: "#dff3e8" };
  return <BadgeSeplag {...meta} size="sm" fontWeight />;
}
  ...
  const celulaComEspecificacao = (
    ...
    item.id === filtrados[0]?.id ? (
      <SpecArea metadata={metadata}>
        <div className="prototype-temporarios-quadro-spec-cell">{content}</div>
      </SpecArea>
    ) : (
      content
    );
  ...
  const columns: ColumnMetaSeplag<QuadroListaRow>[] = [
    {
      ...
      body: (item) =>
        celulaComEspecificacao(
          ...
          <div className="prototype-temporarios-quadro-table-main">
            <button
              className="prototype-temporarios-quadro-link"
      ...
      body: (item) =>
        celulaComEspecificacao(
          ...
          <div className="prototype-temporarios-quadro-table-main">
            <button
              type="button"
              className="prototype-temporarios-quadro-orgao-summary-button"
    ...
    },
    {
      ...
      body: (item) => {
        ...
        return (
          <SpecArea metadata={quadroColumnSpecifications.Situação}>
            <div className="prototype-temporarios-quadro-spec-status">
              <BadgeSeplag
  ...
  const renderHistoricoVersoes = (item: QuadroListaRow) => {
    ...
    return (
      <SpecArea metadata={quadroColumnSpecifications["Histórico de versões"]}>
        <section className="prototype-temporarios-quadro-version-history">
          {versoes.length ? (
            <>
              <div className="prototype-temporarios-quadro-version-table-container">
                <table className="prototype-temporarios-quadro-version-table">
                  <colgroup>
                    ...
                    {versoesPaginadas.map((versao) => {
                      ...
                      return (
                        <tr key={`${item.codigo}-${versao.versao}`}>
                          ...
                          <td>
                            <div className="prototype-temporarios-quadro-actions">
                              <BotaoIconSeplag
                                type="button"
              ...
              <footer className="prototype-temporarios-quadro-version-pagination">
                <Paginator
            ...
            <p className="prototype-temporarios-quadro-version-empty">
  ...
  const renderControleHistorico = (item: QuadroListaRow) => {
    ...
    return (
      <SpecArea metadata={quadroColumnSpecifications["Histórico de versões"]}>
        <button
          type="button"
          className="prototype-temporarios-quadro-expander"
          aria-label={
            aberto
    ...
    >
      <div className="prototype-temporarios-quadro-page prototype-temporarios-quadro-page-current">
        ...
        <section className="prototype-temporarios-quadro-card">
          <div className="prototype-temporarios-quadro-filters prototype-temporarios-quadro-library-filters">
            <SpecArea metadata={quadroFilterSpecifications.Quadro}>
              <div className="prototype-temporarios-quadro-spec-control">
                <TextFieldSeplag
                  name="busca"
                  control={control}
                  label="Quadro"
          ...
          <div className="prototype-temporarios-quadro-table-toolbar">
          ...
          <SpecArea metadata={quadroTableSpecification}>
            <div className="prototype-temporarios-quadro-table prototype-temporarios-quadro-library-table">
        ...
        >
          <label className="prototype-temporarios-quadro-delete-reason">
            <span>
...
function DistribuicaoOrgaosModal({
  ...
  return (
    <ModalSeplag
      ...
      customFooter={
        <div className="prototype-temporarios-quadro-modal-library-actions">
          <BotaoVoltarSeplag
            label="Fechar"
            icon="pi pi-times"
            onClick={onClose}
    ...
    >
      <div className="col-12 prototype-temporarios-quadro-distribution-modal">
        <header>
          <div>
        ...
        {quadro.distribuicaoOrgaos.length ? (
          <div className="prototype-temporarios-quadro-distribution-modal-list">
...
function HistoricoVersaoModal({
  ...
  return (
    ...
    >
      <div className="col-12 prototype-temporarios-quadro-version-modal">
        <div className="prototype-temporarios-quadro-version-modal-status">
          <BadgeSeplag
            label={metaStatus.label}
            color={metaStatus.color}
            bg={metaStatus.bg}
...
function QuadroAutorizadoModal({
  ...
  return (
    <ModalSeplag
      ...
      titulo={
        <div>
          ...
          <small className="prototype-temporarios-quadro-modal-library-subtitle">
    ...
    >
      <div className="col-12 prototype-temporarios-quadro-modal-status">
        <SpecArea metadata={quadroColumnSpecifications.Situação}>
          <BadgeSeplag
            label={status.label}
            color={status.color}
      ...
      <div className="col-12 prototype-temporarios-quadro-modal-body">
        <section>
          <h3>Identificação</h3>
          <dl>
            <div>
        ...
        <section>
          ...
          {!quantitativos.length && orgaosDoQuadro(registro).length > 1 && (
            <p className="prototype-temporarios-quadro-modal-orgs">
...
function QuadroAutorizadoForm({
  ...
  return (
    <div className="prototype-temporarios-quadro-page">
      ...
      />
      <form onSubmit={submit} className="prototype-temporarios-quadro-form">
        <BaseLegalVinculada
        ...
        <section>
          ...
          <div className="prototype-temporarios-quadro-fields">
            <DropdownFieldSeplag
              name="vinculo"
              control={control}
              label="Tipo de vínculo"
            ...
            <DropdownFieldSeplag
              ...
              itemTemplate={(option) => (
                <span
                  className={
                    option.indisponivel
                      ? "prototype-temporarios-quadro-cargo-indisponivel"
                      : undefined
                  }
                  aria-label={option.motivoIndisponibilidade || undefined}
                >
                  <span className="prototype-temporarios-quadro-cargo-indisponivel-label">
                    {option.label}
                  </span>
                  {option.quadroCodigo && (
                    <span className="prototype-temporarios-quadro-cargo-indisponivel-badge">
                      {option.quadroCodigo}
                    </span>
                  )}
                </span>
        ...
        <div className="prototype-temporarios-quadro-form-paired">
        <section>
          <header>
          ...
          <div className="prototype-temporarios-quadro-scope">
            <NumberFieldSeplag
              name="quantidade"
              control={control}
              label={
            ...
            />
            {novaVersao && (
              <div className="prototype-temporarios-quadro-comparison">
        ...
        <section>
          ...
          <div className="prototype-temporarios-quadro-library-section prototype-temporarios-quadro-simple-vigencia">
            <div className="prototype-temporarios-quadro-vigencia-date">
              <DateFieldSeplag
                name="dataAtivacao"
                control={control}
            ...
            <div className="prototype-temporarios-quadro-simple-status" aria-live="polite">
              <i
                className={
                  "pi " +
                  (situacaoPrevia === "Agendado"
        ...
        <footer className="prototype-temporarios-quadro-form-actions prototype-temporarios-quadro-form-actions--flow">
          <BotaoVoltarSeplag
            label="Cancelar"
            icon="pi pi-times"
            onClick={onBack}
...
function QuadroAutorizadoDetalhe({
  ...
  return (
    <div className="prototype-temporarios-quadro-page">
      <header className="prototype-temporarios-quadro-header">
        <div>
          ...
          <div className="prototype-temporarios-quadro-title-line">
            ...
            <span
              className={`prototype-temporarios-quadro-status ${situacaoClass[situacao]}`}
            >
              {situacao}
            </span>
          </div>
        ...
        <div className="prototype-temporarios-quadro-header-actions">
          ...
          {situacao === "Vigente" && (
            <button
              className="prototype-temporarios-quadro-primary"
              onClick={() =>
                navigate(`${BASE_PATH}/${registro.id}/nova-versao`)
              }
            >
      ...
      <section className="prototype-temporarios-quadro-detail-kpis">
        <article>
          <span>Autorizadas</span>
          <strong>{registro.autorizadas}</strong>
        </article>
      ...
      <div className="prototype-temporarios-quadro-detail-grid">
        <section className="prototype-temporarios-quadro-detail-card">
          <header>
      ...
      <section className="prototype-temporarios-quadro-detail-card prototype-temporarios-quadro-history">
        <header>
          <div>
          ...
          <tbody>
            ...
            {registro.versao > 1 && (
              <tr>
                ...
                <td>
                  <span className="prototype-temporarios-quadro-status is-closed">
                    Substituída
                  </span>
                </td>
              </tr>
