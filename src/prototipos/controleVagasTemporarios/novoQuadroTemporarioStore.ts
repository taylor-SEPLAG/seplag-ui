...
export interface QuadroTemporarioCadastro {
 ...
 observacoes: string; criadoEm: string; dataAtivacao?: string; vagasReais: number; situacao: "ATIVO" | "EXTINTO" | "ENCERRADO";
}
...
export function listarQuadrosTemporarios(): QuadroTemporarioCadastro[] {
 ...
 return json ? JSON.parse(json).map((item: QuadroTemporarioCadastro) => ({ ...item, vagasReais: item.vagasReais ?? 0, situacao: item.situacao ?? "ATIVO", cargos: item.cargos ?? item.certame.cargos.map(cargo => ({ ...cargo, modoControle: cargo.aceitaCadastroReserva ? "SEM_LIMITE" : "LIMITADO", limiteVagas: cargo.aceitaCadastroReserva ? undefined : cargo.quantidadeVagas })) })) : [];
}
...
export function salvarQuadroTemporario(certame: Certame, observacoes: string, dataAtivacao?: string) {
 ...
 const registro: QuadroTemporarioCadastro = {
  ...
  dataAtivacao, observacoes: observacoes.trim(), vagasReais: 0, situacao: "ATIVO", criadoEm: new Date().toISOString(),
 };
