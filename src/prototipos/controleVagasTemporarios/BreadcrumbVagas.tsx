...
export function BreadcrumbVagas() {
  ...
  const atalho = partes.length === 1 && tela === "quadro-autorizado"
    ? { label: "Vagas Contratos Temporários", icon: "pi pi-list", destino: base + "/vagas" }
    : partes.length === 1 && tela === "vagas"
      ? { label: "Quadro Contratos Temporários", icon: "pi pi-table", destino: base + "/quadro-autorizado" }
      : null;

  return <div className="breadcrumb-vagas-temporarios">
